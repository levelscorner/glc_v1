...
| **E** | `setup/README.md` — Bot Framework Emulator + Azure setup walkthrough | 3–4 | unclaimed | — | `<name>/teams-setup-readme` |
...
| **H** | Demo video (Loom unlisted, ~5 min) — `pytest` green + emulator round-trip | 2–3 | unclaimed | — | (no branch; URL posted in upstream PR body) |
| **I** | PR review + lint polish — ruff/mypy/boundary check green before upstream PR opens | 2–3 | unclaimed | — | `<name>/teams-lint-polish` |
| **J** | CI scorecard pre-flight — run all checks locally, post scorecard screenshot in axiom chat before opening upstream PR | 1–2 | unclaimed | — | (no branch; verification only) |
